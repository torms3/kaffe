...
def up(bottoms, num_output, ks, s, lr_mult=1, bias_term=False):
    ...
    # Factorizing 3D convolution.
    factor = None
    if ks[0] > 1:
        factor = L.Convolution(bottoms,
            num_output=num_output, kernel_size=[1, ks[1], ks[2]], stride=s,
            weight_filler=dict(type="msra"), bias_filler=dict(type="constant"),
            param=[dict(lr_mult=lr_mult)], bias_term=False)
        bottoms = factor
        ks = [ks[0], 1, 1]
        s = 1
    ...
    return L.Convolution(bottoms,
		...
		weight_filler=dict(type="msra"), bias_filler=dict(type="constant"),
        param=param, bias_term=bias_term), factor


def down(bottoms, num_output, ks, s, lr_mult=1):
    """Implement deconvolution/upsample."""
    # Factorizing 3D deconvolution.
    factor = None
    if ks[0] > 1:
        factor = L.Deconvolution(bottoms,
            convolution_param=dict(
    		num_output=num_output, kernel_size=[1, ks[1], ks[2]], stride=s,
    		weight_filler=dict(type="msra"), bias_filler=dict(type="constant"),
            bias_term=False), param=[dict(lr_mult=lr_mult)])
        bottoms = factor
        ks = [ks[0], 1, 1]
        s = 1
    return L.Deconvolution(bottoms,
		...
		num_output=num_output, kernel_size=ks, stride=s,
		weight_filler=dict(type="msra"), bias_filler=dict(type="constant")),
        param=[dict(lr_mult=lr_mult), dict(lr_mult=lr_mult)]), factor
    ...
    for i in xrange(len(nfeatures)):  # Time steps.
        for j in xrange(i, len(nfeatures[i])):  # Layers.
            if is_input(i,j):
                ...
                prev, factor = up(tops[i][j], nfeatures[i][j+1], sizes[j-i], strides[j-i], bias_term=True)
                net[name] = prev
                if factor is not None:
                    net[name+'-f'] = factor
                tops[i][j+1] = prev
            elif is_output(i,j):
                net['output'], factor = down(tops[i-1][j], nfeatures[i][j], sizes[j-i], strides[j-i])
                if factor is not None:
                    net['output-f'] = factor
            elif is_valid(i,j):
                postfix = '{},{}'.format(i,j)
                ...
                if has_feedback(i,j):
                    prev, factor = down(tops[i-1][j], nfeatures[i][j], sizes[j-i], strides[j-i])
                    net['deconv'+postfix] = prev
                    if factor is not None:
                        net['deconv'+postfix+'-f'] = factor
                    bottoms.append(prev)
                ...
                if is_valid(i,j+1):
                    ...
                    prev, factor = up(tops[i][j], nfeatures[i][j+1], sizes[j-i], strides[j-i], bias_term=b)
                    net[name] = prev
                    if factor is not None:
                        net[name+'-f'] = factor
                    tops[i][j+1] = prev
